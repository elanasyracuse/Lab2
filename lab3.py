...
# st.title("💬 Lab 3A – Kid-Friendly Chatbot (20-exchange buffer + smart follow-ups)")
...
#     st.session_state.chat_history = []  # list[{"role": "...", "content": "..."}]
# if "last_user_question" not in st.session_state:
#     st.session_state.last_user_question = None  # track the last non-yes/no question
...
# def is_greeting(text: str) -> bool:
#     """Detect simple greetings/introductions like 'Hi', 'I'm Therese', 'My name is...'. """
#     return bool(GREETING_PAT.match(text.strip()))

# def is_question(text: str) -> bool:
#     """Lightweight question detector for kid-style queries."""
#     t = text.strip().lower()
#     return (
#         t.endswith("?")
#         or t.startswith((
...
#     # 1) YES → expand the last real question and re-ask follow-up
#     if is_yes(prompt):
#         if st.session_state.last_user_question:
#             with st.chat_message("assistant"):
#                 expand_instruction = (
#                     "The user said YES to more info. "
#                     "Please add more helpful details about your last answer to this question: "
#                     f"\"{st.session_state.last_user_question}\". "
#                     "Keep it simple for a 10-year-old. Use short sentences and tiny examples."
#                 )
#                 payload = build_payload_messages(pending_user_message=expand_instruction)
...
#             st.session_state.chat_history.append({"role": "assistant", "content": full})
#         else:
#             # No previous question to expand—prompt them gently
#             with st.chat_message("assistant"):
#                 msg = "Sure! Tell me what you want to learn about."
#                 st.markdown(msg)
#             st.session_state.chat_history.append({"role": "assistant", "content": msg})

#     # 2) NO → ask what else to help with (no follow-up)
#     elif is_no(prompt):
#         with st.chat_message("assistant"):
#             msg = "Okay! What can I help you with?"
#             st.markdown(msg)
#         st.session_state.chat_history.append({"role": "assistant", "content": msg})
#         # optional: clear last question so we don't expand the wrong thing later
#         # st.session_state.last_user_question = None

#     # 3) Greeting/intro → friendly welcome (no follow-up)
#     elif is_greeting(prompt) and not is_question(prompt):
#         st.session_state.last_user_question = None
#         with st.chat_message("assistant"):
#             # Keep it warm and kid-friendly
#             msg = "Hi! It’s nice to meet you! How can I help you today?"
#             st.markdown(msg)
#         st.session_state.chat_history.append({"role": "assistant", "content": msg})

#     # 4) Real question → answer and ask follow-up
#     else:
#         st.session_state.last_user_question = prompt if is_question(prompt) else None
#         with st.chat_message("assistant"):
#             payload = build_payload_messages(pending_user_message=None)  # user already appended
#             full = stream_and_render(payload, append_follow_up=is_question(prompt))
#         st.session_state.chat_history.append({"role": "assistant", "content": full})
